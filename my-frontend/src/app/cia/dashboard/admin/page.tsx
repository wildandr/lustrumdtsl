...
export default function DashboardAdmin() {
    ...
    const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE3MDgxMTIzMTAsImV4cCI6MTcxMzI5NjMxMH0.db2v2NM80xLldbtuE3vbEGiQxxTwMN-_ORPa72BdtYY";

    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await axios.get('http://127.0.0.1:5001/teams/sbc/', {
            headers: {
              ...
              <tbody className=" text-black rounded-xl">
                {registrations.map((registration, index) => (
                  ...
                  >
                    ...
                    <td className="font-semibold px-2 border-r border-ciaGreen border-opacity-10">
                      {"Perlu Konfirmasi"}
                    </td>
                    <td className="px-2">{registration.team.event_id}</td>
                    <td className="px-[0.6rem] py-2 rounded-r-xl">
                      <div className="flex-col flex gap-2 md:flex-row">
                        ...
                        <button className="bg-[#E25933] text-white text-[13px] lg:text-[16px] rounded-md px-1 py-1 w-full">
                          Tolak
                        </button>
                      </div>
                    </td>
