...
export function Isi() {
    ...
    return (
        ...
        <div className='min-h-screen pt-[17%] p-6 flex flex-col items-center justify-center bg-black relative overflow-x-clip'>
            <div className='absolute min-h-screen w-full'>
                <Image
                 src={'/assets/lustrum/logo_kmtsl_vektor.svg'}
                 className={`w-[42rem] h-auto absolute -top-[30%] right-0 z-50 hidden lg:block`}
                 width={1000}
                 height={1000}
                 alt={''}
                 style={{}}
            ...
            <div className='flex flex-col md:w-[60vw] lg:w-[71vw] relative z-[100]'>
                <p className='text-xs md:text-sm lg:text-base font-LibreBaskerville font-bold bg-gradient-to-b text-transparent bg-clip-text from-[#F2D87A] to-[#E0B620]'>CIA X CRAFT</p>
                <p className='text-2xl md:text-3xl lg:text-5xl font-LibreBaskerville bg-gradient-to-b text-transparent bg-clip-text from-[#F2D87A] to-[#E0B620] mt-4'>Civil in Action x Civil Engineering Advance Software Training</p>
                <p className='text-xs md:text-sm lg:text-xl font-LibreBaskerville bg-gradient-to-b text-transparent bg-clip-text from-[#F2D87A] to-[#E0B620] mt-4'>Civil In Action adalah event tahunan yang diselenggarakan oleh mahasiswa Departemen Teknik Sipil dan Lingkungan (DTSL) Fakultas Teknik UGM yang bertujuan sebagai wadah untuk mengembangkan ilmu pengetahuan dan keprofesian di bidang teknik sipil dan lingkungan bagi semua pihak yang terlibat</p>
                <Link href={`cia`} style={{border: "1px solid var(--gold, #F2D87A)"}} className='w-fit flex flex-row items-center border bg-gradient-to-b bg-clip-text from-[#F2D87A] to-[#E0B620] text-gold-500 px-4 py-2 mt-5 md:mt-8 rounded-xl font-bold text-xs lg:text-base hover:opacity-80'>Website CIA X CRAFT
                    <span>
