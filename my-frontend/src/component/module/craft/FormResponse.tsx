...
export default function DetailUser({ params }: { params: any }) {
    ...
    async function downloadFilesAsZip() {
        ...
        const combinedCsv = parse(participantData, {
            fields: Object.keys(participantData[0]),
        });
        ...
        if (participant.ktm) {
            ...
            zip.file(
                participant.ktm.split("/").pop() ?? "ktm_default_name",
                ktmData
            );
        }
        ...
        if (participant.payment_proof) {
            ...
            zip.file(
                participant.payment_proof.split("/").pop() ??
                    "payment_proof_default_name",
                paymentProofData
            );
        }
